...
class ScaleJitter(nn.Module):
    ...
    def forward(
        ...
        if target is not None:
            ratio_w = new_width / orig_width
            ratio_h = new_height / orig_height
            boxes_02 = target["boxes"][:, 0::2]
            boxes_12 = target["boxes"][:, 1::2]

            mult_tensor_w = torch.tensor(
                boxes_02 * ratio_w, dtype=torch.short)
            mult_tensor_h = torch.tensor(
                boxes_12 * ratio_h, dtype=torch.short)
            zeros = torch.zeros_like(mult_tensor_w)

            if torch.sum(mult_tensor_w == zeros) == mult_tensor_w.numel():
                print('mult_tensor_w', ratio_w)
            if torch.sum(mult_tensor_h == zeros) == mult_tensor_h.numel():
                print('mult_tensor_h', ratio_h)

            target["boxes"][:, 0::2] = mult_tensor_w
            target["boxes"][:, 1::2] = mult_tensor_h
            if "masks" in target:
                target["masks"] = F.resize(
                    target["masks"], [new_height,
                                      new_width], interpolation=InterpolationMode.NEAREST
                )
        print(target['boxes'])
        return image, target
