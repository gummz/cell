from matplotlib.colors import ListedColormap
import matplotlib.pyplot as plt
from os.path import join
from os import listdir, makedirs
import os
import numpy as np

from src.data.constants import FIG_DIR, RAW_DATA_DIR, RAW_FILE_DIMENSIONS, TIMEPOINTS, RAW_FILES
import tifffile as tiff


# Set working directory to file location
abspath = os.path.abspath(__file__)
dname = os.path.dirname(abspath)
os.chdir(dname)

operation = '3d_plot'
save = join(FIG_DIR, operation)
try:
    makedirs(save)
except FileExistsError:
    pass

# files = RAW_FILES[START_IDX:]
files = listdir(RAW_DATA_DIR)
# Filter out already used files
files = [file for file in files if file in RAW_FILES]
file_paths = [join(RAW_DATA_DIR, file) for file in files]
file_paths_tiff = [file for file in file_paths if '.lsm' in file]

fig, ax = plt.subplots(subplot_kw={'projection': '3d'})

file_idx = 0
file_path = file_paths_tiff[file_idx]
with tiff.TiffFile(file_path) as f:
    pages = np.array(f.pages[::2])

    pages = pages.reshape(
        (TIMEPOINTS[file_idx], RAW_FILE_DIMENSIONS[files[file_idx]]))
    timepoint = pages[0, :]
    timepoint = [slice.asarray()[0, :, :] for slice in timepoint]
    timepoint = np.array(timepoint)
    timepoint = np.moveaxis(timepoint, 0, 2)
    size = 512
    r = range(size)
    r38 = np.linspace(0, 1, 38)
    X, Y, Z = np.meshgrid(r, r, r38)
    print(X.shape, Y.shape, Z.shape)

    # Choose colormap
    cmap = plt.cm.RdBu

    # Get the colormap colors
    my_cmap = cmap(np.arange(cmap.N))

    # Set alpha
    my_cmap[:, -1] = np.linspace(0, 1, cmap.N)

    # Create new colormap
    my_cmap = ListedColormap(my_cmap)

    # X = timepoint[0]
    # Y = timepoint[1]
    # Z = timepoint[2]
    # # ax.plot_surface(mesh)
    timepoint = np.where(timepoint > 100, timepoint, 0)
    ax.scatter(X, Y, Z, c=timepoint[0:size, 0:size, :], cmap=my_cmap)

    plt.savefig(join(save, 'plot.jpg'))

print('3d_plot.py complete')
