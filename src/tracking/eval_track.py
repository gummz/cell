...
def eval_track(tracked_centroids, time_range, filename, location):
    ...
    for t, (_, frame) in zip(time_range, frames):
        ...
        if True:  # not os.path.exists(join(location, f'{name}.png')):
            ...
            images = output_raw_images(location, path, channels, t, name)
...
def output_raw_images(location, path, channels, t, name):
...
def output_figure(filename, t, cells_scale, tubes_mip, combined, save):
    ...
    plt.imshow(tubes_mip, cmap='Greens')
...
if __name__ == '__main__':
    ...
    files = utils.add_ext(files)  # add extensions (.lsm etc.)
    name = files[0]  # c.PRED_FILE

    load_location = join(c.DATA_DIR, c.PRED_DIR,
                         name, 'tracked_centroids.pkl')
    ...
    frames = tuple(pd.unique(tracked_centroids['frame']))
    time_range = range(min(frames), max(frames))
